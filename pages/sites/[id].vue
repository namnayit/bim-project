...
const highlightedAssets = computed(() => {
  if (!searchQuery.value) {
    return displayedAssets.value.map((asset) => ({
      ...asset,
  ...
  return displayedAssets.value
    ...
    .map((asset) => ({
      ...asset,
      highlightedName: asset.name.replace(regex, `<mark class="bg-yellow-300 dark:bg-yellow-600">$1</mark>`),
      highlightedType: asset.type.replace(regex, `<mark class="bg-yellow-300 dark:bg-yellow-600">$1</mark>`),
      highlightedQuantity: asset.quantity.toString().replace(regex, `<mark class="bg-yellow-300 dark:bg-yellow-600">$1</mark>`),
    }));
});
        ...
        <div class="max-w-screen-xl mx-auto pt-4">
            ...
            <div class="flex items-center justify-between mb-4">
                <div class="flex items-center">
                    ...
                    <h2 class="text-2xl font-semibold capitalize single-head">{{ siteName }}</h2>
                </div>
                <button @click="toggleAddSlideOver" class="text-white bg-blue-600 hover:bg-blue-700 font-medium rounded-lg px-4 py-2 ad-ast">Add Asset</button>
            </div>
            ...
            <!-- Materials and Equipment List with Images -->
            <div>
                <ul>
                    <li
                        v-for="asset in highlightedAssets"
                        :key="asset.id"
                        class="bg-zinc-50 dark:bg-zinc-800 rounded-lg p-3 border border-zinc-300 dark:border-zinc-600 flex justify-between items-center mb-3"
                    >
                        <div class="flex items-center space-x-4">
                            <!-- Image Preview -->
                            <div v-if="asset.image_url" class="w-12 h-12 overflow-hidden rounded-lg border dark:border-zinc-600 image-p">
                            <img :src="asset.image_url" alt="Asset Image" class="object-cover w-full h-full" />
                            </div>
                            <div v-else class="w-12 h-12 bg-gray-200 dark:bg-gray-700 flex items-center justify-center rounded-lg" />

                            <!-- Highlighted Asset Fields -->
                            <div>
                            <span class="text-lg font-medium text-zinc-900 dark:text-white capitalize" v-html="asset.highlightedName ?? ''" />
                            <span class="block text-sm text-zinc-600 dark:text-zinc-400" v-html="`Quantity: ${asset.highlightedQuantity ?? ''}`" />
                            </div>
                        </div>
                        ...
                        <div class="flex space-x-4 act-flex">
                            ...
                            <button @click="toggleEditSlideOver(asset)" class="text-white p-2 rounded-full bg-blue-600 hover:bg-blue-800 transition">
                                <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" viewBox="0 0 512 512" fill="none">
                                    <path
                            ...
                            <button @click="toggleTransferSlideOver(asset)" class="text-white p-2 rounded-full bg-green-600 hover:bg-green-800 transition">
                                <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" viewBox="0 0 512 512" fill="none">
                                    <path
                                        fill="none"
                                        stroke="currentColor"
