...
export function useAsset() {
    ...
    const fetchAssets = async (siteId) => {
        const { data, error } = await supabase.from("assets").select("*").eq("site_id", siteId).order("name", { ascending: true }); // Orders assets alphabetically by name
    ...
    const fetchOtherSites = async (currentSiteId) => {
        const { data, error } = await supabase.from("sites").select("*").neq("id", currentSiteId);
        if (error) {
    ...
    const addAsset = async (newAsset) => {
        ...
        if (existing) {
            return { success: false, error: "An asset with this name already exists for the site" };
        }else{
            const { data, error } = await supabase.from("assets").insert(newAsset).select("id").single();
             if (error) {
                return { success: false, error: `Error adding asset: ${error.message}` };
            }
    ...
    const updateAssetImageUrl = async (assetId, imageUrlPath) => {
        const { error } = await supabase.from("assets").update({ image_url: imageUrlPath }).eq("id", assetId);
        if (error) {
        ...
        return new Promise((resolve, reject) => {
            new Compressor(file, {
                ...
                success: async (compressedFile) => {
                    ...
                    const { data, error } = await supabase.storage.from("assets-images").upload(filePath, compressedFile);

                    if (error) {
                        imageUploadStatus.value = `Failed to upload image: ${error.message}`;
                        reject(error);
                    } else {
                        const { data } = supabase.storage.from("assets-images").getPublicUrl(filePath);
    ...
    const addOrUpdateAssetImage = async (assetId, file) => {
        try {
            ...
            if (!imageUrl) throw new Error("Failed to retrieve image URL after upload");
            console.log("assetID!!!!!: ", assetId);
            // Update the asset with the new image URL if `assetId` is provided
            if (assetId) {
                const { error } = await supabase.from("assets").update({ image_url: imageUrl }).eq("id", assetId);
    ...
    const deleteAssetImage = async (assetId) => {
        try {
            ...
            const { error: storageError } = await supabase.storage.from("assets-images").remove([filePath]);

            if (storageError) throw new Error(`Failed to delete image from storage: ${storageError.message}`);

            const { error: updateError } = await supabase.from("assets").update({ image_url: null }).eq("id", assetId);

            if (updateError) throw new Error(`Failed to update asset image URL: ${updateError.message}`);
    ...
    const transferAssetToMultipleSites = async (
        ...
        try {
            ...
            if (totalQty > currentQuantity) throw new Error("Transfer quantity exceeds available.");
            ...
            const transferPromises = transfers.map(async ({ targetSiteId, quantity }) => {
            ...
            if (remaining === 0) {
                ...
                const { data: mainAsset, error: existingMainAssetError } = await supabase
                    .from("assets")
                    .select("*")
                ...
                if (existingMainAssetError && existingMainAssetError.code !== "PGRST116") {
                ...
                if (mainAsset) {
                    ...
                    const { error: insertMainAsset } = await supabase.from("assets").insert({
                        site_id: mainSite.id,
                        name,
                        quantity: 0,
                        type,
    ...
    const deleteAsset = async (assetId: number) => {
        try {
            ...
            if (error) {
            return { success: false, error: `Error deleting asset: ${error.message}` };
            }
